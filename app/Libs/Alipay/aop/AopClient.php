    ...
    */
    public function pageExecute($request, $httpmethod = "POST")
    {
        ...
        if (method_exists($request, "getNeedEncrypt") && $request->getNeedEncrypt()) {
            ...
            \Illuminate\Support\Facades\Log::info('加密');
            // 执行加密
            $enCryptContent = encrypt2($apiParams['biz_content'], $this->encryptKey);
            \Illuminate\Support\Facades\Log::info('加密完成');
    ...
    private function encryptJSONSignSource($request, $responseContent)
    {
        ...
        $bizContent = decrypt2($parsetItem->encryptContent, $this->encryptKey);
